...
//        scoreLabel.position = CGPoint(x: (-size.width/2) + 200, y: self.frame.size.height/2 - 100)
        ...
        homeButton.position = CGPoint(x: -100, y: -200)
        homeButton.name = "home"
        homeButton.zPosition = 1
        homeButton.size = CGSize(width: playButton.size.width, height: playButton.size.height)
...
//        }
        ...
        if let location = touch?.location(in: self)
        {
            ...
            for node in theNodes
            {
                ...
                if node.name == "home"
                {
                    //let menuVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "mainPage") as! GameViewController
            ...
            for data in result as! [NSManagedObject] {
                if(data.value(forKey: "score") as! Int32 == -1) {
                    ...
                    do {
                        ...
                        print("Error! Failed to save the score!")
                    }
                }
                else if(data.value(forKey: "name") as? String == name){
                    do {
                        try context.save()
                    }
                    catch {
                        print("Error! Failed to override the score!")
                    }
                }
            }
        } catch {
            print("Error! Failed to fetch the requested data")
        }
        bulletTimer?.invalidate()
        gameTimer?.invalidate()
    }}
